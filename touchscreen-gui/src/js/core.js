...
IS24.core.topNavigation = {
	slideIn: (function () {
		...
		function init() {
			...
			if (!slideInContainer || initialized) { return; }
			...
			initialized = true; // prevents duplicate click handlers (and the resulting simultaneous open/close in the same event)
		}
	...
	touchHover: (function () {
		...
		// check if the slide-in navigation is active by testing if one of the slide-in triggers is being displayed
		function hasSlideInNavigation() {
			var slideInTriggers = (document.querySelectorAll) ? document.querySelectorAll('[class*="top-navigation__slide-in-trigger--"]') : [],
				hasSlideIn = false,
				i;
			
			for (i=0; i<slideInTriggers.length; i++) {
				if (slideInTriggers[i].offsetWidth !== 0) {
					hasSlideIn = true;
				}
			}
		...
		function tabHandler(event) {
			...
			if (hasSlideInNavigation()) { return; } // skip touch handling when slide-in navigation is displayed instead of the tab navigation
		...
		function init() {
			...
			if (hoverTabLabels.length === 0 || hoverTabs.length === 0 || initialized) { return; }
...
/*
 *  popoverTrigger
 *  is a jquery helper function to register popover listener on the document body.
 *  the popover will stay open until the body is clicked.
 *
 *  author: sstiehl
 *  version 0.0.2
 *
 *
 */
jQuery.popoverTrigger = (function ($) {
  "use strict";
  ...
  function closeAll() {
    $.each(triggeredElements, function (index, el) {
      el.popover("hide");
    });
  ...
  return function popoverLoader(trigger) {
    var body = $("body");
    ...
    body.on("click", trigger, function (event) {
      ...
      if (!el.is(lastClickedElement)) {
        createPopover(el);
        el.popover("show");
        triggeredElements.push(el);
